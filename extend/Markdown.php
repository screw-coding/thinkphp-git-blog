...
//        return $Extra->text($text);
    ...
    {
        if (env("ENV") != "development" && $this->enableCache && !empty($objdata)) {
    ...
    {
        if (env("ENV") != "development" && $this->enableCache) {
    ...
    public function deleteFileExt($fileName)
    {
        //xxx.eeee.ext->xxx eeee ext len=3 ->xxx.eeee
        $pics = explode('.', $fileName);
        $len = count($pics);
        if ($len > 1) {
            unset($pics[$len - 1]);
